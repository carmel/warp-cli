...
func (w *Warping) Run() PingDelaySet {
	if len(w.ips) == 0 {
		return w.csv
	}
...
func (w *Warping) warpingHandler(ip *UDPAddr) {
	...
	w.bar.Describe(strconv.Itoa(nowAble))
	if recv == 0 {
		return
	}
	data := &PingData{
...
func (w *Warping) appendIPData(data *PingData) {
	w.m.Lock()
	defer w.m.Unlock()
	w.csv = append(w.csv, CloudflareIPData{
		PingData: data,
	})
}
...
func InitHandshakePacket() {
	if ReservedString != "" {
		if PrivateKey == "" {
			log.Fatalln("ReservedEmptyError")
		}
		r, err := ParseReservedString(ReservedString)
		if err != nil {
			log.Fatalln("ReservedParseError" + err.Error())
		}
		reserved = r
	}
	...
	if err != nil {
		log.Fatalln("PrivateKeyParseError" + err.Error())
	}

	pub, err := getNoisePublicKeyFromBase64(PublicKey)
	if err != nil {
		log.Fatalln("PublicKeyParseError" + err.Error())
	}
...
func buildHandshakePacket(pri device.NoisePrivateKey, pub device.NoisePublicKey) []byte {
	...
	if err != nil {
		log.Fatalln("HandshakePacketBuildFailed" + err.Error())
	}
...
func encodeBase64ToHex(key string) (string, error) {
	...
	if err != nil {
		return "", errors.New("Base64Invalid" + key)
	}
	if len(decoded) != 32 {
		return "", errors.New("NoiseKeyInvalid" + key)
	}
