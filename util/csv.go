...
func ExportCsv(data []CloudflareIPData) {
	...
	if err != nil {
		log.Fatalf("CreateFileFailed: %v", err)
		return
	}
	defer fp.Close()
	w := csv.NewWriter(fp)
...
func (s PingDelaySet) Print() {
	...
	log.Printf(headFormat, "IP:Port", "PacketLossRate", "Latency")
	for i := 0; i < PrintNum; i++ {
		log.Printf(dataFormat, dataString[i][0], dataString[i][1], dataString[i][2])
	}
	if !noOutput() {
		log.Println("WriteResultToFileDone", Output)
	}
}
